...
class DatabaseObat:
 ...
 # ==================== DATA OBAT ====================
    ...
    def tanggal_kadaluarsa(self):
        self.kursor.execute("SELECT plu, nama_produk, tanggal_kadaluarsa FROM data_obat")
        rows = self.kursor.fetchall()
        ...
        for plu, nama, tgl_str in rows:
            if not tgl_str:
                continue  # skip kalau kosong/null
            try:
                exp_date = datetime.datetime.strptime(tgl_str, "%Y-%m-%d").date()
            except ValueError:
                continue  # skip kalau format salah

            selisih = (exp_date - today).days
            if 0 <= selisih <= 30:  # mendekati 1 bulan
                rows_1bulan.append((plu, nama, tgl_str))
            elif 31 <= selisih <= 60:  # mendekati 2 bulan
                rows_2bulan.append((plu, nama, tgl_str))

        return rows, rows_2bulan, rows_1bulan
